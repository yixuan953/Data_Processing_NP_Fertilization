...
# Output path:
N_inorg_5arcm_path = 'N_Inorg_amount_5arcm'
N_inorg_05d_path = 'N_Inorg_amount_05d'

os.makedirs(N_inorg_5arcm_path, exist_ok=True)
os.makedirs(N_inorg_05d_path, exist_ok=True)
...
for crop in crop_namelist:
    ...
    data_Urea = nc_file['Urea_N_fert'][:].transpose("year", "lat", "lon")  # read and if needed correct shape to year, lat, lon
    data_Inorg = nc_file['Inorganic_N_fert'][:].transpose("year", "lat", "lon")  # read and if needed correct shape to year, lat, lon
...
N_inorg_app_rate_05d_path = 'N_Inorg_app_rate_05d'
os.makedirs(N_inorg_app_rate_05d_path, exist_ok=True)
...
for crop in crop_namelist:
    ...
    data_Urea = nc_Urea['Urea_N_amount'][:].transpose("year", "lat", "lon")  # read and if needed correct shape to year, lat, lon

    nc_Inorg = xr.open_dataset(Inorg_05d)    
    data_Inorg = nc_Inorg['Inorg_N_amount'][:].transpose("year", "lat", "lon")  # read and if needed correct shape to year, lat, lon

    nc_Total_Inorg = xr.open_dataset(Total_Inorg_05d)    
    data_Total_Inorg = nc_Total_Inorg['Total inorganic N amount'][:].transpose("year", "lat", "lon")  # read and if needed correct shape to year, lat, lon
    ...
    output_N_urea_app_rate_05d = os.path.join(N_inorg_app_rate_05d_path, f"N_urea_app_rate_{crop}_1961-2020.nc")
    ...
    output_N_inorg_app_rate_05d = os.path.join(N_inorg_app_rate_05d_path, f"N_inorg_app_rate_{crop}_1961-2020.nc")
    ...
    output_N_total_inorg_app_rate_05d = os.path.join(N_inorg_app_rate_05d_path, f"N_total_inorg_app_rate_{crop}_1961-2020.nc")
